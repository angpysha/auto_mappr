...
class ClassBodyBuilder extends MapBodyBuilderBase {
  ...
  void _assertParamFieldCanBeIgnored(FormalParameterElement param, PropertyAccessorElement sourceField) {
    final sourceFieldName = sourceField.displayName;
  ...
  ConstructorElement? _findBestConstructor(InterfaceType classType, {String? forcedConstructor}) {
    if (forcedConstructor != null) {
      final selectedConstructor = classType.allConstructors.firstWhereOrNull((c) => c.name == forcedConstructor);
    ...
    final allConstructors = classType.allConstructors.where((c) => c.isPublic);
